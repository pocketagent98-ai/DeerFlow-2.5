...
| **🛡️ z.ai paid-model safety** | `providers/discovery.py` | z.ai uses exactly its three fully-free models (`glm-4.7-flash`, `glm-4.5-flash`, `glm-4.6v-flash`) and NOTHING else — the models listing is informational only: it can never add a paid model, and it can never remove the free ones (live-verified: a valid key chats with glm-4.7-flash even while the listing omits it). |
